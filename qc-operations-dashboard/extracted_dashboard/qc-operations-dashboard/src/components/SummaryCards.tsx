...
const SummaryCards: React.FC<SummaryCardsProps> = ({ data }) => {
  const stats = useMemo(() => {
    ...
    // Calculate average processing time (for orders being picked)
    const pickingOrders = data.filter(o => {
      const status = o.partial_status.toUpperCase();
      return status === 'PICKING' || status === 'PICKING_WITH_PACKING' || status === 'DOING' || status === 'PROCESSING';
    });
