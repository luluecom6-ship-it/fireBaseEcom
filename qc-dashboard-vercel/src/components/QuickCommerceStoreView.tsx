...
import { Order, Store, STATUS_ORDER, STATUS_LABELS, getStatusColor, getAgeBucket, getOrderAgeMinutes, extractStoreCode, mapStatus } from '../types';
...
const QuickCommerceStoreView: React.FC<QuickCommerceStoreViewProps> = ({ data, stores }) => {
  ...
  const { matrixData, orderDetails } = useMemo(() => {
    ...
    const details: Record<string, Record<string, Array<{ jobNumber: string; ageBucket: string }>>> = {};
  ...
  const getCellColor = (count: number): string => {
    ...
    if (count <= 1) return 'rgba(239, 68, 68, 0.15)';
    if (count <= 3) return 'rgba(239, 68, 68, 0.25)';
  ...
  const handleCellClick = (
    ...
    status: string,
    store: Store,
    orders: Array<{ jobNumber: string; ageBucket: string }>
  ) => {
