...
async function startServer() {
  ...
  // GAS Proxy Route
    ...
    try {
      ...
      // Hierarchy: 1. query param (trust client intent), 2. specialized V2 env, 3. general GAS env
      let rawUrl = String(req.query.gasUrl || "").trim();
      
      if (!rawUrl || rawUrl === "undefined" || !rawUrl.startsWith("http")) {
        rawUrl = (process.env.V2_GAS_URL || process.env.VITE_V2_GAS_URL || process.env.GAS_API_URL || process.env.VITE_GAS_API_URL || "").trim();
      }
      
      // Final hardcoded fallback if still empty
      if (!rawUrl || rawUrl === "undefined" || !rawUrl.startsWith("http")) {
        rawUrl = "https://script.google.com/macros/s/AKfycbziSK-a3_zBsoEPHBe1Yaz-pTEYtnZyuHdTPhziDSlB3Vhn8DZ0qaPLICnb9eY_ptj5/exec";
      }

      console.log(`[Proxy] Target URL prefix: ${rawUrl.substring(0, 60)}...`);
